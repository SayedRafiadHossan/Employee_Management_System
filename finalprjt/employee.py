...
def Add():
    ...
    try:
        sql = "INSERT INTO  Employee (empID,Name,Salary,Tax) VALUES (%s, %s, %s, %s)"
        val = (empID,Name,salary,Tax)
...
def update():
    ...
    try:
        sql = "Update  Employee set Name= %s,Salary= %s,tax= %s where empID= %s"
        val = (empID,name,salary,Tax)
        mycursor.execute(sql, val)
        mydb.commit()
...
def show():
    mydb = mysql.connector.connect(host='localhost', user="root", password="dipa123", database="prjt")
    mycursor = mydb.cursor()
    sql = "SELECT empID,Name,Salary,Tax, Annual_Income, Bonus FROM Employee"
    mycursor.execute(sql)
    records = mycursor.fetchall()
    print(records)

    for i, (empID, Name, Salary, Tax, Annual_Income, Bonus) in enumerate(records, start=1):
        listBox.insert("", "end", values=(empID,Name,Salary,float(Tax)*float(Salary),float(Salary)*12,float(Salary)*0.5))
        mydb.close()
...
win = Tk()
win.title("Employee")
win.maxsize(width=1180, height=500)
win.minsize(width=1180, height=500)
...
lbl4 = Label(win, text="Tax", bg="navy blue", fg="white", width=15, height=1, font=("Arial", 10, "bold"))
...
cols = ('Employee ID', 'Employee Name', 'Salary', 'Tax', 'Annual_Income', 'Bonus')
...
listBox.column("Tax",width=190)
listBox.column("Annual_Income",width=190)
listBox.column("Bonus",width=190)
